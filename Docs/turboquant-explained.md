...
This document is the plain-English version of the TurboQuant work in this repository.

If you are not deep in `llama.cpp`, `ggml`, or GPU kernels, start here.
...
- prompt-ingest scaling work
  A fuller evaluation should chart how prompt throughput falls as context length rises, so the boundary between "memory win" and "prefill wall" is visible.
...
That is the reason this project is worth keeping and packaging, rather than treating it as a speculative experiment.
