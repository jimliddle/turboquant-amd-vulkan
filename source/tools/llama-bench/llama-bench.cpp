...
struct cmd_params {
    ...
    std::vector<int>                 turboquant_group_size;
    std::vector<int>                 turboquant_residual_bits;
...
static void print_usage(int /* argc */, char ** argv) {
    ...
    printf("  --kv-tq-group-size <n>                      (default: %s)\n", join(cmd_params_defaults.turboquant_group_size, ",").c_str());
    printf("  --kv-tq-residual-bits <n>                   (default: %s)\n", join(cmd_params_defaults.turboquant_residual_bits, ",").c_str());
...
static llama_kv_cache_codec llama_kv_cache_codec_from_name(const std::string & s) {
    if (s == "none") {
    ...
    return (llama_kv_cache_codec) -1;
}

static llama_turboquant_runtime llama_turboquant_runtime_from_name(const std::string & s) {
    if (s == "auto") {
    ...
    return (llama_turboquant_runtime) -1;
}
    ...
    for (int i = 1; i < argc; i++) {
        ...
        try {
            if (arg == "-h" || arg == "--help") {
                ...
                auto p = string_split<std::string>(argv[i], split_delim);
                for (const auto & v : p) {
                    auto codec = llama_kv_cache_codec_from_name(v);
                    if ((int) codec < 0) {
                        invalid_param = true;
                        break;
                    }
                    params.kv_cache_codec.push_back(codec);
                }
                if (invalid_param) {
                    break;
                }
                ...
                auto p = string_split<std::string>(argv[i], split_delim);
                for (const auto & v : p) {
                    auto runtime = llama_turboquant_runtime_from_name(v);
                    if ((int) runtime < 0) {
                        invalid_param = true;
                        break;
                    }
                    params.turboquant_runtime.push_back(runtime);
                }
                if (invalid_param) {
                    break;
                }
                ...
                auto p = parse_int_range(argv[i]);
                params.turboquant_group_size.insert(params.turboquant_group_size.end(), p.begin(), p.end());
                ...
                auto p = parse_int_range(argv[i]);
                params.turboquant_residual_bits.insert(params.turboquant_residual_bits.end(), p.begin(), p.end());
...
struct cmd_params_instance {
    ...
    int                turboquant_group_size;
    int                turboquant_residual_bits;
    bool               turboquant_qjl;
...
static std::vector<cmd_params_instance> get_cmd_params_instances(const cmd_params & params) {
    ...
    for (const auto & kvcodec : params.kv_cache_codec)
    for (const auto & tqrt : params.turboquant_runtime)
    for (const auto & tqgs : params.turboquant_group_size)
    for (const auto & tqrb : params.turboquant_residual_bits)
    for (const auto & tqqjl : params.turboquant_qjl)
    for (const auto & tqfb : params.turboquant_allow_fallback)
    ...
    for (const auto & pl : params.poll) {
        for (const auto & n_prompt : params.n_prompt) {
            ...
            cmd_params_instance instance = {
                ...
                /* .type_k       = */ tk,
                /* .type_v       = */ tv,
                /* .kv_cache_codec = */ kvcodec,
                /* .turboquant_runtime = */ tqrt,
                /* .turboquant_group_size = */ tqgs,
                /* .turboquant_residual_bits = */ tqrb,
                /* .turboquant_qjl = */ tqqjl,
                /* .turboquant_allow_fallback = */ tqfb,
                /* .n_threads    = */ nt,
                /* .cpu_mask     = */ cm,
                /* .cpu_strict   = */ cs,
                /* .poll         = */ pl,
...
struct test {
    ...
    static const std::vector<std::string> & get_fields() {
        static const std::vector<std::string> fields = {
            ...
            "type_k",         "type_v",         "kv_codec",      "kv_tq_runtime",  "kv_tq_group_size",
            "kv_tq_residual_bits",             "kv_tq_qjl",     "kv_tq_fallback", "n_gpu_layers",
    ...
    static field_type get_field_type(const std::string & field) {
        if (field == "build_number" || field == "n_batch" || field == "n_ubatch" || field == "n_threads" ||
            field == "poll" || field == "model_size" || field == "model_n_params" || field == "kv_tq_group_size" ||
            field == "kv_tq_residual_bits" || field == "n_gpu_layers" || field == "main_gpu" || field == "n_prompt" ||
            field == "n_gen" || field == "n_depth" || field == "avg_ns" || field == "stddev_ns" ||
            field == "no_op_offload" || field == "n_cpu_moe") {
            return INT;
        }
        if (field == "f16_kv" || field == "kv_tq_qjl" || field == "kv_tq_fallback" || field == "no_kv_offload" ||
            field == "cpu_strict" || field == "flash_attn" || field == "use_mmap" || field == "use_direct_io" ||
...
struct markdown_printer : public printer {
    ...
    static int get_field_width(const std::string & field) {
        ...
        if (field == "kv_tq_runtime") {
            return 13;
        }
        if (field == "kv_tq_group_size" || field == "kv_tq_residual_bits") {
            return 8;
        }
        if (field == "split_mode") {
            return 5;
    ...
    static std::string get_field_display_name(const std::string & field) {
        ...
        if (field == "kv_tq_group_size") {
            return "tq_grp";
        }
        if (field == "kv_tq_residual_bits") {
            return "tq_res";
        }
        if (field == "kv_tq_qjl") {
            return "tq_qjl";
        }
        if (field == "kv_tq_fallback") {
            return "tq_fb";
        }
        if (field == "flash_attn") {
            return "fa";
    ...
    void print_header(const cmd_params & params) override {
        ...
        if (params.turboquant_allow_fallback.size() > 1 || params.turboquant_allow_fallback != cmd_params_defaults.turboquant_allow_fallback) {
            fields.emplace_back("kv_tq_fallback");
        }
