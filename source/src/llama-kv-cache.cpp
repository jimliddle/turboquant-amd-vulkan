...
bool llama_kv_cache::state_read_data(llama_io_read_i & io, uint32_t strm, uint32_t cell_count, const slot_info & sinfo) {
    ...
    io.read_to(&state_codec, sizeof(state_codec));
    io.read_to(&tq_params.group_size, sizeof(tq_params.group_size));
    io.read_to(&tq_params.residual_bits, sizeof(tq_params.residual_bits));
    io.read_to(&tq_qjl, sizeof(tq_qjl));
        ...
        if (cell_count && state_codec == LLAMA_KV_STATE_CODEC_RAW) {
            ...
            for (uint32_t i = 0; i < cell_count; ++i) {
                llama_turboquant_unpack_row(
                        ...
                        row_data.data());
                const size_t dst_offset = sinfo.idxs[0][i] * k_size_row;
                ggml_backend_tensor_set(k, row_data.data(), dst_offset, k_size_row);
            }
        }
    }
