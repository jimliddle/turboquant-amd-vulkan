...
struct ggml_vk_turboquant_profile_state {
    bool enabled = getenv("GGML_VK_TURBOQUANT_PROFILE") != nullptr;
    std::atomic<uint64_t> fa_scalar_count = 0;
    std::atomic<uint64_t> fa_coopmat1_count = 0;
    std::atomic<uint64_t> fa_coopmat2_count = 0;
    std::atomic<uint64_t> fa_tq_scalar_k_count = 0;
    std::atomic<uint64_t> fa_tq_scalar_kv_count = 0;
    std::atomic<uint64_t> fa_tq_cm1_k_count = 0;
    std::atomic<uint64_t> fa_tq_cm1_kv_count = 0;
};

static ggml_vk_turboquant_profile_state g_ggml_vk_turboquant_profile;

static void ggml_vk_turboquant_profile_print() {
    if (!g_ggml_vk_turboquant_profile.enabled) {
        return;
    }

    std::cerr
            << "ggml_vulkan turboquant profile:"
            << " fa_scalar=" << g_ggml_vk_turboquant_profile.fa_scalar_count.load()
            << " fa_coopmat1=" << g_ggml_vk_turboquant_profile.fa_coopmat1_count.load()
            << " fa_coopmat2=" << g_ggml_vk_turboquant_profile.fa_coopmat2_count.load()
            << " tq_scalar_k=" << g_ggml_vk_turboquant_profile.fa_tq_scalar_k_count.load()
            << " tq_scalar_kv=" << g_ggml_vk_turboquant_profile.fa_tq_scalar_kv_count.load()
            << " tq_cm1_k=" << g_ggml_vk_turboquant_profile.fa_tq_cm1_k_count.load()
            << " tq_cm1_kv=" << g_ggml_vk_turboquant_profile.fa_tq_cm1_kv_count.load()
            << std::endl;
}

static bool g_ggml_vk_turboquant_profile_registered = []() {
    std::atexit(ggml_vk_turboquant_profile_print);
    return true;
}();
...
struct vk_device_struct {
    ...
    std::map<vk_fa_pipeline_state, vk_pipeline> pipeline_flash_attn_turboquant_k_f16;
    std::map<vk_fa_pipeline_state, vk_pipeline> pipeline_flash_attn_turboquant_kv_f16;
    std::map<vk_fa_pipeline_state, vk_pipeline> pipeline_flash_attn_turboquant_k_cm1_f16;
    std::map<vk_fa_pipeline_state, vk_pipeline> pipeline_flash_attn_turboquant_kv_cm1_f16;

    std::map<std::pair<uint32_t, uint32_t>, vk_pipeline> pipeline_fa_mask_opt;

    vk_pipeline pipeline_flash_attn_split_k_reduce;
    vk_pipeline pipeline_count_experts;
    vk_pipeline pipeline_turboquant_materialize_f16;
    vk_pipeline pipeline_turboquant_materialize_f32;
    vk_pipeline pipeline_turboquant_materialize_bf16;
...
struct vk_turboquant_materialize_push_constants {
    uint32_t n_row_el;
    uint32_t kv_size;
    uint32_t packed_row_size;
    uint32_t n_rows;
    uint32_t group_size;
...
static_assert(sizeof(vk_turboquant_materialize_push_constants) <= 128, "sizeof(vk_turboquant_materialize_push_constants) must be <= 128");
...
struct ggml_vk_turboquant_resident_cache;
...
static bool ggml_vk_turboquant_prepare_flash_attn_tensor(
        ggml_backend_vk_context * ctx,
        vk_context & subctx,
        const ggml_tensor * tensor,
        uint32_t kv_rows,
        vk_subbuffer & tensor_buf);
static bool ggml_vk_turboquant_prepare_flash_attn_packed_k(
        ggml_backend_vk_context * ctx,
        const ggml_tensor * tensor,
        uint32_t kv_rows,
        uint32_t & packed_row_size,
...
#define CREATE_FA(TYPE, NAMELC, FAPATH, SUFFIX) \
    ...
    for (auto &fa : device->pipeline_flash_attn_turboquant_k_f16) {
        if (fa.first.path != FA_TURBOQUANT_K) {
            continue;
        }
        ...
        if (aligned) {
            if (f32acc) {
                ggml_vk_create_pipeline(device, fa.second, "flash_attn_f32_f16_aligned_turboquant_k_f32acc", flash_attn_f32_f16_turboquant_k_f16_len, flash_attn_f32_f16_turboquant_k_f16_data, "main", 7, sizeof(vk_flash_attn_push_constants), {Br, 1, 1}, get_fa_spec_constants(fa.first), Bc, true, !fa_ds, (!fa_ds ? fa_sgs : 0));
            } else {
                ggml_vk_create_pipeline(device, fa.second, "flash_attn_f32_f16_aligned_turboquant_k_f16acc", flash_attn_f32_f16_turboquant_k_f16_f16acc_len, flash_attn_f32_f16_turboquant_k_f16_f16acc_data, "main", 7, sizeof(vk_flash_attn_push_constants), {Br, 1, 1}, get_fa_spec_constants(fa.first), Bc, true, !fa_ds, (!fa_ds ? fa_sgs : 0));
            }
        } else {
            if (f32acc) {
                ggml_vk_create_pipeline(device, fa.second, "flash_attn_f32_f16_turboquant_k_f32acc", flash_attn_f32_f16_turboquant_k_f16_len, flash_attn_f32_f16_turboquant_k_f16_data, "main", 7, sizeof(vk_flash_attn_push_constants), {Br, 1, 1}, get_fa_spec_constants(fa.first), 1, true, !fa_ds, (!fa_ds ? fa_sgs : 0));
            } else {
                ggml_vk_create_pipeline(device, fa.second, "flash_attn_f32_f16_turboquant_k_f16acc", flash_attn_f32_f16_turboquant_k_f16_f16acc_len, flash_attn_f32_f16_turboquant_k_f16_f16acc_data, "main", 7, sizeof(vk_flash_attn_push_constants), {Br, 1, 1}, get_fa_spec_constants(fa.first), 1, true, !fa_ds, (!fa_ds ? fa_sgs : 0));
            }
        }
    }
    for (auto &fa : device->pipeline_flash_attn_turboquant_kv_f16) {
        if (fa.first.path != FA_TURBOQUANT_KV) {
            continue;
        }
        const uint32_t Br = fa.first.Br;
        const uint32_t Bc = fa.first.Bc;
        ...
        if (aligned) {
            if (f32acc) {
                ggml_vk_create_pipeline(device, fa.second, "flash_attn_f32_f16_aligned_turboquant_kv_f32acc", flash_attn_f32_f16_turboquant_kv_f16_len, flash_attn_f32_f16_turboquant_kv_f16_data, "main", 7, sizeof(vk_flash_attn_push_constants), {Br, 1, 1}, get_fa_spec_constants(fa.first), Bc, true, !fa_ds, (!fa_ds ? fa_sgs : 0));
            } else {
                ggml_vk_create_pipeline(device, fa.second, "flash_attn_f32_f16_aligned_turboquant_kv_f16acc", flash_attn_f32_f16_turboquant_kv_f16_f16acc_len, flash_attn_f32_f16_turboquant_kv_f16_f16acc_data, "main", 7, sizeof(vk_flash_attn_push_constants), {Br, 1, 1}, get_fa_spec_constants(fa.first), Bc, true, !fa_ds, (!fa_ds ? fa_sgs : 0));
            }
        } else {
            if (f32acc) {
                ggml_vk_create_pipeline(device, fa.second, "flash_attn_f32_f16_turboquant_kv_f32acc", flash_attn_f32_f16_turboquant_kv_f16_len, flash_attn_f32_f16_turboquant_kv_f16_data, "main", 7, sizeof(vk_flash_attn_push_constants), {Br, 1, 1}, get_fa_spec_constants(fa.first), 1, true, !fa_ds, (!fa_ds ? fa_sgs : 0));
            } else {
                ggml_vk_create_pipeline(device, fa.second, "flash_attn_f32_f16_turboquant_kv_f16acc", flash_attn_f32_f16_turboquant_kv_f16_f16acc_len, flash_attn_f32_f16_turboquant_kv_f16_f16acc_data, "main", 7, sizeof(vk_flash_attn_push_constants), {Br, 1, 1}, get_fa_spec_constants(fa.first), 1, true, !fa_ds, (!fa_ds ? fa_sgs : 0));
            }
        }
    }
    for (auto &fa : device->pipeline_flash_attn_turboquant_k_cm1_f16) {
        if (fa.first.path != FA_TURBOQUANT_K_CM1) {
            continue;
        }
        const uint32_t Br = fa.first.Br;
        const uint32_t Bc = fa.first.Bc;
        ...
        if (aligned) {
            if (f32acc) {
                ggml_vk_create_pipeline(device, fa.second, "flash_attn_f32_f16_aligned_turboquant_k_cm1_f32acc", flash_attn_f32_f16_turboquant_k_f16_cm1_len, flash_attn_f32_f16_turboquant_k_f16_cm1_data, "main", 7, sizeof(vk_flash_attn_push_constants), {Br, 1, 1}, get_fa_spec_constants(fa.first), Bc, true, !fa_ds, (!fa_ds ? fa_sgs : 0));
            } else {
                ggml_vk_create_pipeline(device, fa.second, "flash_attn_f32_f16_aligned_turboquant_k_cm1_f16acc", flash_attn_f32_f16_turboquant_k_f16_f16acc_cm1_len, flash_attn_f32_f16_turboquant_k_f16_f16acc_cm1_data, "main", 7, sizeof(vk_flash_attn_push_constants), {Br, 1, 1}, get_fa_spec_constants(fa.first), Bc, true, !fa_ds, (!fa_ds ? fa_sgs : 0));
            }
        } else {
            if (f32acc) {
                ggml_vk_create_pipeline(device, fa.second, "flash_attn_f32_f16_turboquant_k_cm1_f32acc", flash_attn_f32_f16_turboquant_k_f16_cm1_len, flash_attn_f32_f16_turboquant_k_f16_cm1_data, "main", 7, sizeof(vk_flash_attn_push_constants), {Br, 1, 1}, get_fa_spec_constants(fa.first), 1, true, !fa_ds, (!fa_ds ? fa_sgs : 0));
            } else {
                ggml_vk_create_pipeline(device, fa.second, "flash_attn_f32_f16_turboquant_k_cm1_f16acc", flash_attn_f32_f16_turboquant_k_f16_f16acc_cm1_len, flash_attn_f32_f16_turboquant_k_f16_f16acc_cm1_data, "main", 7, sizeof(vk_flash_attn_push_constants), {Br, 1, 1}, get_fa_spec_constants(fa.first), 1, true, !fa_ds, (!fa_ds ? fa_sgs : 0));
            }
        }
    }
    for (auto &fa : device->pipeline_flash_attn_turboquant_kv_cm1_f16) {
        if (fa.first.path != FA_TURBOQUANT_KV_CM1) {
            continue;
        }
        const uint32_t Br = fa.first.Br;
        const uint32_t Bc = fa.first.Bc;
        ...
        if (aligned) {
            if (f32acc) {
                ggml_vk_create_pipeline(device, fa.second, "flash_attn_f32_f16_aligned_turboquant_kv_cm1_f32acc", flash_attn_f32_f16_turboquant_kv_f16_cm1_len, flash_attn_f32_f16_turboquant_kv_f16_cm1_data, "main", 7, sizeof(vk_flash_attn_push_constants), {Br, 1, 1}, get_fa_spec_constants(fa.first), Bc, true, !fa_ds, (!fa_ds ? fa_sgs : 0));
            } else {
                ggml_vk_create_pipeline(device, fa.second, "flash_attn_f32_f16_aligned_turboquant_kv_cm1_f16acc", flash_attn_f32_f16_turboquant_kv_f16_f16acc_cm1_len, flash_attn_f32_f16_turboquant_kv_f16_f16acc_cm1_data, "main", 7, sizeof(vk_flash_attn_push_constants), {Br, 1, 1}, get_fa_spec_constants(fa.first), Bc, true, !fa_ds, (!fa_ds ? fa_sgs : 0));
            }
        } else {
            if (f32acc) {
                ggml_vk_create_pipeline(device, fa.second, "flash_attn_f32_f16_turboquant_kv_cm1_f32acc", flash_attn_f32_f16_turboquant_kv_f16_cm1_len, flash_attn_f32_f16_turboquant_kv_f16_cm1_data, "main", 7, sizeof(vk_flash_attn_push_constants), {Br, 1, 1}, get_fa_spec_constants(fa.first), 1, true, !fa_ds, (!fa_ds ? fa_sgs : 0));
            } else {
                ggml_vk_create_pipeline(device, fa.second, "flash_attn_f32_f16_turboquant_kv_cm1_f16acc", flash_attn_f32_f16_turboquant_kv_f16_f16acc_cm1_len, flash_attn_f32_f16_turboquant_kv_f16_f16acc_cm1_data, "main", 7, sizeof(vk_flash_attn_push_constants), {Br, 1, 1}, get_fa_spec_constants(fa.first), 1, true, !fa_ds, (!fa_ds ? fa_sgs : 0));
            }
        }
    }
...
#undef CREATE_GLU
    ...
    ggml_vk_create_pipeline(device, device->pipeline_turboquant_materialize_f16,  "turboquant_materialize_f16",  turboquant_materialize_f16_len,  turboquant_materialize_f16_data,  "main", 3, sizeof(vk_turboquant_materialize_push_constants), {64, 1, 1}, {}, 1, true);
    ggml_vk_create_pipeline(device, device->pipeline_turboquant_materialize_f32,  "turboquant_materialize_f32",  turboquant_materialize_f32_len,  turboquant_materialize_f32_data,  "main", 3, sizeof(vk_turboquant_materialize_push_constants), {64, 1, 1}, {}, 1, true);
    ggml_vk_create_pipeline(device, device->pipeline_turboquant_materialize_bf16, "turboquant_materialize_bf16", turboquant_materialize_bf16_len, turboquant_materialize_bf16_data, "main", 3, sizeof(vk_turboquant_materialize_push_constants), {64, 1, 1}, {}, 1, true);
...
static void ggml_vk_flash_attn(ggml_backend_vk_context * ctx, vk_context& subctx, const ggml_tensor * q, const ggml_tensor * k, const ggml_tensor * v, const ggml_tensor * mask, const ggml_tensor * sinks, ggml_tensor * dst) {
    ...
    uint32_t tq_group_size = 0;
    uint32_t tq_residual_bits = 0;
    bool tq_qjl = false;
    bool tq_transposed = false;
    uint32_t tq_v_packed_row_size = 0;
    uint32_t tq_v_group_size = 0;
    uint32_t tq_v_residual_bits = 0;
    bool tq_v_qjl = false;
    bool tq_v_transposed = false;
    ...
    use_turboquant_k = tuning_params.path == FA_SCALAR &&
            k->type == GGML_TYPE_F16 &&
            ggml_vk_turboquant_prepare_flash_attn_packed_k(ctx, k, KV, tq_packed_row_size, tq_group_size, tq_residual_bits, tq_qjl, tq_transposed, k_buf);
    if (use_turboquant_k) {
        fa_pipeline_state.path = FA_TURBOQUANT_K;
    }
    if (!use_turboquant_k && tuning_params.path == FA_COOPMAT1 &&
            k->type == GGML_TYPE_F16 &&
            ggml_vk_turboquant_prepare_flash_attn_packed_k(ctx, k, KV, tq_packed_row_size, tq_group_size, tq_residual_bits, tq_qjl, tq_transposed, k_buf)) {
        use_turboquant_k = true;
        fa_pipeline_state.path = FA_TURBOQUANT_K_CM1;
    }
    use_turboquant_v = (tuning_params.path == FA_SCALAR || tuning_params.path == FA_COOPMAT1) &&
            v->type == GGML_TYPE_F16 &&
            ggml_vk_turboquant_prepare_flash_attn_packed_k(ctx, v, KV, tq_v_packed_row_size, tq_v_group_size, tq_v_residual_bits, tq_v_qjl, tq_v_transposed, v_buf) &&
            !tq_v_transposed;
    if (use_turboquant_k && use_turboquant_v) {
        fa_pipeline_state.path = tuning_params.path == FA_COOPMAT1 ? FA_TURBOQUANT_KV_CM1 : FA_TURBOQUANT_KV;
    }

    switch (fa_pipeline_state.path) {
        case FA_SCALAR:            g_ggml_vk_turboquant_profile.fa_scalar_count++; break;
        case FA_TURBOQUANT_K:      g_ggml_vk_turboquant_profile.fa_tq_scalar_k_count++; break;
        case FA_TURBOQUANT_KV:     g_ggml_vk_turboquant_profile.fa_tq_scalar_kv_count++; break;
        case FA_TURBOQUANT_K_CM1:  g_ggml_vk_turboquant_profile.fa_tq_cm1_k_count++; break;
        case FA_TURBOQUANT_KV_CM1: g_ggml_vk_turboquant_profile.fa_tq_cm1_kv_count++; break;
        case FA_COOPMAT1:          g_ggml_vk_turboquant_profile.fa_coopmat1_count++; break;
        case FA_COOPMAT2:          g_ggml_vk_turboquant_profile.fa_coopmat2_count++; break;
    }
...

    {
        ...
        auto &pipelines =
                fa_pipeline_state.path == FA_TURBOQUANT_KV_CM1 ? ctx->device->pipeline_flash_attn_turboquant_kv_cm1_f16 :
                fa_pipeline_state.path == FA_TURBOQUANT_KV     ? ctx->device->pipeline_flash_attn_turboquant_kv_f16 :
                fa_pipeline_state.path == FA_TURBOQUANT_K_CM1  ? ctx->device->pipeline_flash_attn_turboquant_k_cm1_f16 :
                fa_pipeline_state.path == FA_TURBOQUANT_K      ? ctx->device->pipeline_flash_attn_turboquant_k_f16 :
    ...
    if (!use_turboquant_k) {
        ggml_vk_turboquant_prepare_flash_attn_tensor(ctx, subctx, k, KV, k_buf);
    }
    if (!use_turboquant_v) {
        ggml_vk_turboquant_prepare_flash_attn_tensor(ctx, subctx, v, KV, v_buf);
    }
    ...
    const uint32_t tq_nb11 = use_turboquant_k ? tq_packed_row_size : k_stride;
    const uint32_t tq_nb12 = use_turboquant_k ? tq_group_size : (uint32_t) nbk2;
    const uint32_t tq_nb13 = use_turboquant_k ? (tq_residual_bits | (tq_qjl ? (1u << 8) : 0u)) : (uint32_t) nbk3;
    const uint32_t tq_nb21 = use_turboquant_v ? tq_v_packed_row_size : v_stride;
    const uint32_t tq_nb22 = use_turboquant_v ? tq_v_group_size : (uint32_t) nbv2;
    const uint32_t tq_nb23 = use_turboquant_v ? (tq_v_residual_bits | (tq_v_qjl ? (1u << 8) : 0u)) : (uint32_t) nbv3;
    ...
    const vk_flash_attn_push_constants pc = { N, KV,
                                              ...
                                              tq_nb11, tq_nb12, tq_nb13,
                                              tq_nb21, tq_nb22, tq_nb23,
...
static vk_pipeline ggml_vk_get_turboquant_materialize_pipeline(vk_device & device, ggml_type type) {
    switch (type) {
        case GGML_TYPE_F16:
            return device->pipeline_turboquant_materialize_f16;
        case GGML_TYPE_F32:
            return device->pipeline_turboquant_materialize_f32;
        case GGML_TYPE_BF16:
            return device->pipeline_turboquant_materialize_bf16;
        default:
            return nullptr;
    }
}

struct ggml_vk_turboquant_upload_cache {
    vk_device_ref device;
    vk_buffer row_indices;
};

struct ggml_vk_turboquant_resident_cache {
    vk_device_ref device;
    const ggml_tensor * tensor = nullptr;
    size_t base_view_offs = 0;
    vk_buffer packed_rows;
    vk_buffer dense_rows;
    vk_buffer row_indices_full;
    uint32_t kv_size = 0;
    size_t packed_row_size = 0;
    size_t dense_row_size = 0;
    uint32_t row_indices_full_count = 0;
...
struct ggml_vk_turboquant_cache_key {
    const ggml_tensor * root = nullptr;
    size_t view_offs = 0;

    bool operator==(const ggml_vk_turboquant_cache_key & other) const {
        return root == other.root && view_offs == other.view_offs;
    }
};

struct ggml_vk_turboquant_cache_key_hash {
    size_t operator()(const ggml_vk_turboquant_cache_key & key) const {
        return std::hash<const ggml_tensor *>{}(key.root) ^ (std::hash<size_t>{}(key.view_offs) << 1);
    }
};

static thread_local std::unordered_map<ggml_vk_turboquant_cache_key, ggml_vk_turboquant_resident_cache, ggml_vk_turboquant_cache_key_hash> g_ggml_vk_turboquant_resident_caches;

static ggml_vk_turboquant_cache_key ggml_vk_turboquant_get_cache_key(const ggml_tensor * tensor) {
    ggml_vk_turboquant_cache_key key;
    key.root = tensor;
    key.view_offs = 0;

    while (key.root && key.root->view_src) {
        key.view_offs += key.root->view_offs;
        key.root = key.root->view_src;
    }

    return key;
}

static ggml_vk_turboquant_resident_cache * ggml_vk_turboquant_find_resident_cache(const ggml_tensor * tensor) {
    const auto key = ggml_vk_turboquant_get_cache_key(tensor);
    auto it = g_ggml_vk_turboquant_resident_caches.find(key);
    if (it == g_ggml_vk_turboquant_resident_caches.end()) {
        return nullptr;
    }
    return &it->second;
}

static bool ggml_vk_turboquant_prepare_upload_buffer(
        ggml_vk_turboquant_upload_cache & cache,
        vk_device & device,
        vk_buffer & buffer,
        size_t required_size) {
    const size_t alloc_size = ggml_vk_align_size(required_size, size_t(256));
    ...
    if (cache.device.expired() || cache.device.lock() != device) {
        ggml_vk_destroy_buffer(cache.row_indices);
        cache.device = device;
    }

    if (buffer == nullptr || buffer->size < alloc_size) {
...
static bool ggml_vk_turboquant_prepare_resident_buffer(
        ggml_vk_turboquant_resident_cache & cache,
        vk_device & device,
        const ggml_tensor * resident_tensor,
        size_t base_view_offs,
        uint32_t kv_size,
        size_t packed_row_size) {
    const size_t required_size = size_t(kv_size) * packed_row_size;
    const size_t alloc_size = ggml_vk_align_size(required_size, size_t(256));

    if (cache.device.expired() || cache.device.lock() != device || cache.tensor != resident_tensor ||
            cache.base_view_offs != base_view_offs ||
            cache.kv_size != kv_size || cache.packed_row_size != packed_row_size) {
        ggml_vk_destroy_buffer(cache.packed_rows);
        ggml_vk_destroy_buffer(cache.dense_rows);
        ggml_vk_destroy_buffer(cache.row_indices_full);
        cache.device = device;
        cache.tensor = resident_tensor;
        cache.base_view_offs = base_view_offs;
        cache.kv_size = kv_size;
        cache.packed_row_size = packed_row_size;
        cache.dense_row_size = 0;
        cache.row_indices_full_count = 0;
        cache.group_size = 0;
        cache.residual_bits = 0;
        cache.qjl = false;
    }
    ...
    if (cache.packed_rows == nullptr || cache.packed_rows->size < alloc_size) {
        ggml_vk_destroy_buffer(cache.packed_rows);
        cache.packed_rows = ggml_vk_create_buffer_check(
                device,
                alloc_size,
                vk::MemoryPropertyFlagBits::eHostVisible | vk::MemoryPropertyFlagBits::eHostCoherent,
                vk::MemoryPropertyFlagBits::eDeviceLocal | vk::MemoryPropertyFlagBits::eHostVisible | vk::MemoryPropertyFlagBits::eHostCoherent);
    ...
    return cache.packed_rows != nullptr && cache.packed_rows->ptr != nullptr;
}

static bool ggml_vk_turboquant_prepare_dense_buffer(
        ggml_vk_turboquant_resident_cache & cache,
        vk_device & device,
        size_t dense_row_size,
        uint32_t kv_size) {
    const size_t required_size = dense_row_size * kv_size;
    const size_t alloc_size = ggml_vk_align_size(required_size, size_t(256));

    if (cache.dense_rows == nullptr || cache.dense_rows->size < alloc_size || cache.dense_row_size != dense_row_size) {
        ggml_vk_destroy_buffer(cache.dense_rows);
        cache.dense_rows = ggml_vk_create_buffer_check(device, alloc_size, {vk::MemoryPropertyFlagBits::eDeviceLocal});
        cache.dense_row_size = dense_row_size;
    }

    return alloc_size == 0 || cache.dense_rows != nullptr;
}

static bool ggml_vk_turboquant_prepare_full_row_indices(
        ggml_vk_turboquant_resident_cache & cache,
        vk_device & device,
        uint32_t kv_size) {
    const size_t required_size = size_t(kv_size) * sizeof(uint32_t);
    const size_t alloc_size = ggml_vk_align_size(required_size, size_t(256));
    ...
    if (cache.row_indices_full == nullptr || cache.row_indices_full->size < alloc_size || cache.row_indices_full_count < kv_size) {
        ggml_vk_destroy_buffer(cache.row_indices_full);
        cache.row_indices_full = ggml_vk_create_buffer_check(
                device,
                alloc_size,
                vk::MemoryPropertyFlagBits::eHostVisible | vk::MemoryPropertyFlagBits::eHostCoherent,
                vk::MemoryPropertyFlagBits::eDeviceLocal | vk::MemoryPropertyFlagBits::eHostVisible | vk::MemoryPropertyFlagBits::eHostCoherent);
        if (cache.row_indices_full == nullptr || cache.row_indices_full->ptr == nullptr) {
            return false;
        }
        auto * ptr = static_cast<uint32_t *>(cache.row_indices_full->ptr);
        for (uint32_t i = 0; i < kv_size; ++i) {
            ptr[i] = i;
        }
        cache.row_indices_full_count = kv_size;
    }

    return true;
}

static ggml_vk_turboquant_resident_cache & ggml_vk_turboquant_get_resident_cache(const ggml_tensor * tensor) {
    return g_ggml_vk_turboquant_resident_caches[ggml_vk_turboquant_get_cache_key(tensor)];
}

static bool ggml_vk_turboquant_dispatch_materialize(
        ggml_backend_vk_context * ctx,
        vk_context & compute_ctx,
        ...
        bool qjl,
        vk_subbuffer dst_buf) {
    const vk_turboquant_materialize_push_constants pc = {
        /* .n_row_el        = */ n_row_el,
        /* .kv_size         = */ kv_size,
        /* .packed_row_size = */ (uint32_t) packed_row_size,
        /* .n_rows          = */ n_rows,
        /* .group_size      = */ group_size,
        /* .residual_bits   = */ residual_bits,
        /* .qjl             = */ qjl ? 1u : 0u,
        /* .transposed      = */ transposed ? 1u : 0u,
    };
    ...
    ggml_vk_dispatch_pipeline(
            ...
            pipeline,
            {
                ggml_vk_subbuffer(ctx, packed_rows, 0),
                ggml_vk_subbuffer(ctx, row_indices, 0),
                dst_buf,
            },
            pc,
            { n_row_el, n_rows, 1 });
...
static bool ggml_vk_turboquant_prepare_flash_attn_tensor(
    ...
    ggml_vk_turboquant_resident_cache & cache = it->second;
    if (cache.packed_rows == nullptr || cache.tensor != key.root || cache.kv_size < kv_rows || cache.packed_row_size == 0 || cache.n_row_el == 0) {
        return false;
    }

    if (cache.dense_rows != nullptr) {
        if (key.view_offs < cache.base_view_offs) {
            return false;
        }
        tensor_buf = ggml_vk_subbuffer(ctx, cache.dense_rows, key.view_offs - cache.base_view_offs);
        return true;
    }

    vk_pipeline pipeline = ggml_vk_get_turboquant_materialize_pipeline(ctx->device, tensor->type);
    if (pipeline == nullptr) {
        return false;
    }

    const size_t dense_row_size = ggml_nbytes(cache.tensor);
    if (!ggml_vk_turboquant_prepare_dense_buffer(cache, ctx->device, dense_row_size, 1) ||
        !ggml_vk_turboquant_prepare_full_row_indices(cache, ctx->device, kv_rows)) {
        return false;
    }
    ...
    if (!ggml_vk_turboquant_dispatch_materialize(
            ctx,
            subctx,
            pipeline,
            tensor,
            cache.packed_rows,
            cache.row_indices_full,
            cache.packed_row_size,
            kv_rows,
            cache.n_row_el,
            cache.kv_size,
            cache.transposed,
            ...
            cache.residual_bits,
            cache.qjl,
            ggml_vk_subbuffer(ctx, cache.dense_rows, 0))) {
        return false;
    }
...
static bool ggml_vk_turboquant_prepare_flash_attn_packed_k(
        ...
        vk_subbuffer & tensor_buf) {
    const auto key = ggml_vk_turboquant_get_cache_key(tensor);
    ggml_vk_turboquant_resident_cache * cache = ggml_vk_turboquant_find_resident_cache(tensor);
    if (cache == nullptr) {
        return false;
    }
    if (cache->packed_rows == nullptr) {
        return false;
    }
    if (cache->tensor != key.root) {
        return false;
    }
    if (cache->kv_size < kv_rows) {
        return false;
    }
    if (cache->packed_row_size == 0) {
        return false;
    }
    if (cache->n_row_el == 0) {
        return false;
    ...
    const size_t dense_row_size = size_t(cache->n_row_el) * ggml_type_size(tensor->type);
    if (dense_row_size == 0 || (key.view_offs % dense_row_size) != 0) {
        return false;
    }
    ...
    const size_t relative_view_offs = key.view_offs - cache->base_view_offs;
    const size_t packed_view_row = relative_view_offs / dense_row_size;
    const size_t packed_view_offs = packed_view_row * cache->packed_row_size;
    if (packed_view_offs >= cache->packed_rows->size) {
        return false;
    }

    packed_row_size = (uint32_t) cache->packed_row_size;
    group_size = cache->group_size;
    residual_bits = cache->residual_bits;
    qjl = cache->qjl;
    transposed = cache->transposed;
    tensor_buf = ggml_vk_subbuffer(ctx, cache->packed_rows, packed_view_offs);
    return true;
}

static bool ggml_backend_vk_turboquant_materialize(
        ...
        size_t n_rows) {
    if (backend == nullptr || tensor == nullptr || packed_rows == nullptr || row_indices == nullptr || n_rows == 0) {
        return false;
    }

    ggml_backend_vk_context * ctx = (ggml_backend_vk_context *) backend->context;
    if (ctx == nullptr || tensor->buffer == nullptr || !ggml_backend_buffer_is_vk(tensor->buffer)) {
        return false;
    }

    vk_pipeline pipeline = ggml_vk_get_turboquant_materialize_pipeline(ctx->device, type);
    if (pipeline == nullptr) {
        return false;
    }

    ggml_backend_vk_buffer_context * dst_buf_ctx = (ggml_backend_vk_buffer_context *) tensor->buffer->context;
    if (dst_buf_ctx == nullptr) {
        return false;
    }

    static thread_local ggml_vk_turboquant_upload_cache upload_cache;
    ggml_vk_turboquant_resident_cache & resident_cache = ggml_vk_turboquant_get_resident_cache(tensor);
    const auto key = ggml_vk_turboquant_get_cache_key(tensor);

    const size_t row_index_bytes = n_rows * sizeof(uint32_t);
    if (!ggml_vk_turboquant_prepare_resident_buffer(resident_cache, ctx->device, key.root, key.view_offs, kv_size, packed_row_size) ||
        !ggml_vk_turboquant_prepare_upload_buffer(upload_cache, ctx->device, upload_cache.row_indices, row_index_bytes)) {
        return false;
    }
    resident_cache.group_size = group_size;
    resident_cache.residual_bits = residual_bits;
    resident_cache.qjl = qjl;
    resident_cache.n_row_el = n_row_el;
    resident_cache.transposed = transposed;

    uint8_t * resident_rows = static_cast<uint8_t *>(resident_cache.packed_rows->ptr);
    for (size_t i = 0; i < n_rows; ++i) {
        if (row_indices[i] >= kv_size) {
            return false;
        }
        std::memcpy(
                resident_rows + size_t(row_indices[i]) * packed_row_size,
                packed_rows + i * packed_row_size,
                packed_row_size);
    }
    std::memcpy(upload_cache.row_indices->ptr, row_indices, row_index_bytes);

    vk_context compute_ctx = ggml_vk_get_compute_ctx(ctx);
    ggml_pipeline_request_descriptor_sets(ctx, pipeline, 1);
    ggml_pipeline_allocate_descriptor_sets(ctx);

    const size_t dst_offset = vk_tensor_offset(tensor) + tensor->view_offs;
    ggml_vk_turboquant_dispatch_materialize(
            ctx,
            compute_ctx,
            pipeline,
            tensor,
            resident_cache.packed_rows,
            upload_cache.row_indices,
            packed_row_size,
            (uint32_t) n_rows,
            n_row_el,
            kv_size,
            ...
            residual_bits,
            qjl,
            ggml_vk_subbuffer(ctx, dst_buf_ctx->dev_buffer, dst_offset));

    ggml_vk_synchronize(ctx);
    return true;
}

static bool ggml_backend_vk_turboquant_sync_shadow(
        ggml_backend_t backend,
        ggml_tensor * tensor,
        uint32_t kv_size,
        uint32_t n_row_el,
    ...
    resident_cache.n_row_el = n_row_el;
    resident_cache.transposed = transposed;
    resident_cache.tensor = key.root;
    ...
    const size_t dense_size = ggml_nbytes(key.root);
    if (dense_size > 0) {
        static thread_local ggml_vk_turboquant_upload_cache upload_cache;
        const size_t row_index_bytes = n_rows * sizeof(uint32_t);
        vk_pipeline pipeline = ggml_vk_get_turboquant_materialize_pipeline(ctx->device, tensor->type);
        if (pipeline == nullptr ||
            !ggml_vk_turboquant_prepare_dense_buffer(resident_cache, ctx->device, dense_size, 1) ||
            !ggml_vk_turboquant_prepare_upload_buffer(upload_cache, ctx->device, upload_cache.row_indices, row_index_bytes)) {
            return false;
        }
        ...
        if (!ggml_vk_turboquant_dispatch_materialize(
                ...
                residual_bits,
                qjl,
                ggml_vk_subbuffer(ctx, resident_cache.dense_rows, 0))) {
            return false;
        }
...
static void * ggml_backend_vk_reg_get_proc_address(ggml_backend_reg_t reg, const char * name) {
    GGML_UNUSED(reg);
    if (strcmp(name, "ggml_backend_vk_turboquant_materialize") == 0) {
        return (void *)ggml_backend_vk_turboquant_materialize;
    }
    if (strcmp(name, "ggml_backend_vk_turboquant_sync_shadow") == 0) {
        return (void *)ggml_backend_vk_turboquant_sync_shadow;
    }
