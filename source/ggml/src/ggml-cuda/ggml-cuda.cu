...
#if defined(GGML_USE_HIP)
static __device__ __forceinline__ uint32_t ggml_cuda_turboquant_read_bits(
        const uint8_t * src,
...
static __device__ __forceinline__ float ggml_cuda_turboquant_fp16_to_fp32(const uint16_t bits) {
    const half value = reinterpret_cast<const half *>(&bits)[0];
    return ggml_cuda_cast<float>(value);
}

template<typename dst_t>
static __device__ __forceinline__ void ggml_cuda_turboquant_store(char * dst, size_t offset, float value) {
    *reinterpret_cast<dst_t *>(dst + offset) = ggml_cuda_cast<dst_t>(value);
}

template<typename dst_t, bool transposed>
static __global__ void ggml_cuda_turboquant_materialize_kernel(
        char * dst,
    ...
    const size_t total = (size_t) n_rows * n_row_el;
    if (linear_index >= total) {
        return;
    }

    const uint32_t row_slot = transposed ? (uint32_t) (linear_index % n_rows) : (uint32_t) (linear_index / n_row_el);
    const uint32_t el = transposed ? (uint32_t) (linear_index / n_rows) : (uint32_t) (linear_index % n_row_el);

    const uint8_t * packed_row = packed_rows + (size_t) row_slot * packed_row_size;
    const uint32_t row_index = row_indices[row_slot];
    const uint32_t group_index = group_size == 0 ? 0 : (el / group_size);
    const uint32_t group_start = group_index * group_size;
    const uint32_t group_el = min(group_size, n_row_el - group_start);
    const size_t sign_bytes_size = (group_el + 7u) / 8u;
    const size_t full_group_bytes =
            sizeof(uint16_t) +
            (size_t) ((group_size + 7u) / 8u) +
            ((qjl && residual_bits > 0) ? (sizeof(uint16_t) + (size_t) ((group_size * residual_bits + 7u) / 8u)) : 0);
    const size_t group_offset = (size_t) group_index * full_group_bytes;

    const uint8_t * group_ptr = packed_row + group_offset;
    const float primary_scale = ggml_cuda_turboquant_fp16_to_fp32(*reinterpret_cast<const uint16_t *>(group_ptr));
    const uint8_t * sign_bytes = group_ptr + sizeof(uint16_t);
    const uint32_t index_in_group = el - group_start;
    const bool positive = ggml_cuda_turboquant_read_bits(sign_bytes, index_in_group, 1) != 0;

    float value = positive ? primary_scale : -primary_scale;
    if (qjl && residual_bits > 0) {
        const uint8_t * residual_scale_ptr = sign_bytes + sign_bytes_size;
        const float residual_scale = ggml_cuda_turboquant_fp16_to_fp32(*reinterpret_cast<const uint16_t *>(residual_scale_ptr));
        const uint8_t * residual_bytes = residual_scale_ptr + sizeof(uint16_t);
        const uint32_t q = ggml_cuda_turboquant_read_bits(residual_bytes, (size_t) index_in_group * residual_bits, residual_bits);
        const uint32_t levels = 1u << residual_bits;
        const float deq = levels <= 1 ? 0.0f : (-1.0f + 2.0f * float(q) / float(levels - 1));
        value += residual_scale * deq;
    }

    const size_t dst_offset = transposed
            ? ((size_t) row_index + (size_t) el * kv_size) * sizeof(dst_t)
            : ((size_t) row_index * n_row_el + el) * sizeof(dst_t);
    ggml_cuda_turboquant_store<dst_t>(dst, dst_offset, value);
}

static bool ggml_cuda_turboquant_materialize_device(
        ...
        const uint32_t * row_indices,
        size_t n_rows) {
    if (backend == nullptr || tensor == nullptr || packed_rows == nullptr || row_indices == nullptr) {
        return false;
    }
    if (!ggml_turboquant_codec_supports_type(type) || tensor->type != type || group_size == 0) {
        return false;
    }
    if (n_rows == 0) {
        return true;
    }
    if (tensor->nb[0] != ggml_type_size(type) || !ggml_is_contiguous(tensor)) {
        return false;
    }
    ...
    ggml_cuda_pool_alloc<uint8_t> packed_rows_dev(cuda_ctx->pool(), n_rows * packed_row_size);
    ggml_cuda_pool_alloc<uint32_t> row_indices_dev(cuda_ctx->pool(), n_rows);

    CUDA_CHECK(cudaMemcpyAsync(
            packed_rows_dev.get(),
            packed_rows,
            n_rows * packed_row_size,
    ...
    const size_t total = n_rows * (size_t) n_row_el;
    const int n_blocks = (int) ((total + block_size - 1) / block_size);
    char * dst = static_cast<char *>(tensor->data);

    switch (type) {
        case GGML_TYPE_F32:
            if (transposed) {
                ggml_cuda_turboquant_materialize_kernel<float, true><<<n_blocks, block_size, 0, cuda_ctx->stream()>>>(
                        dst, packed_rows_dev.get(), row_indices_dev.get(), (uint32_t) n_rows, n_row_el, kv_size,
                        group_size, residual_bits, qjl, packed_row_size);
            } else {
                ggml_cuda_turboquant_materialize_kernel<float, false><<<n_blocks, block_size, 0, cuda_ctx->stream()>>>(
                        dst, packed_rows_dev.get(), row_indices_dev.get(), (uint32_t) n_rows, n_row_el, kv_size,
                        group_size, residual_bits, qjl, packed_row_size);
            }
            break;
        case GGML_TYPE_F16:
            if (transposed) {
                ggml_cuda_turboquant_materialize_kernel<half, true><<<n_blocks, block_size, 0, cuda_ctx->stream()>>>(
                        dst, packed_rows_dev.get(), row_indices_dev.get(), (uint32_t) n_rows, n_row_el, kv_size,
                        group_size, residual_bits, qjl, packed_row_size);
            } else {
                ggml_cuda_turboquant_materialize_kernel<half, false><<<n_blocks, block_size, 0, cuda_ctx->stream()>>>(
                        dst, packed_rows_dev.get(), row_indices_dev.get(), (uint32_t) n_rows, n_row_el, kv_size,
                        group_size, residual_bits, qjl, packed_row_size);
            }
            break;
        case GGML_TYPE_BF16:
            if (transposed) {
                ggml_cuda_turboquant_materialize_kernel<nv_bfloat16, true><<<n_blocks, block_size, 0, cuda_ctx->stream()>>>(
                        dst, packed_rows_dev.get(), row_indices_dev.get(), (uint32_t) n_rows, n_row_el, kv_size,
                        group_size, residual_bits, qjl, packed_row_size);
            } else {
                ggml_cuda_turboquant_materialize_kernel<nv_bfloat16, false><<<n_blocks, block_size, 0, cuda_ctx->stream()>>>(
                        dst, packed_rows_dev.get(), row_indices_dev.get(), (uint32_t) n_rows, n_row_el, kv_size,
                        group_size, residual_bits, qjl, packed_row_size);
            }
            break;
...
static bool ggml_backend_hip_turboquant_materialize(
        ggml_backend_t backend,
        ggml_tensor * tensor,
        ggml_type type,
        uint32_t n_row_el,
        ...
        const uint32_t * row_indices,
        size_t n_rows) {
    return ggml_cuda_turboquant_materialize_device(
            backend,
            tensor,
            type,
            n_row_el,
...
#if defined(GGML_USE_HIP)
    if (strcmp(name, "ggml_backend_hip_turboquant_materialize") == 0) {
        return (void *)ggml_backend_hip_turboquant_materialize;
    }
#endif
    return nullptr;
}
